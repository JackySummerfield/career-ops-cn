"""
gen_dashboard.py — Generate a static HTML dashboard from tracker.csv + job directories.

Usage:
    python util/gen_dashboard.py --user SyntheticUser
    python util/gen_dashboard.py --user SyntheticUser --output dashboard.html
    python util/gen_dashboard.py --user SyntheticUser --vscode   # add VS Code URIs
...
def generate_dashboard(user_dir: Path, use_vscode: bool) -> str:
    ...
    # Sort active: score desc, stage progress, last_updated desc
...
<title>Career Dashboard — {user_dir.name}</title>
<style>{CSS}</style>
</head>
<body>
<h1>💼 Career Dashboard</h1>
<p class="meta">Generated: {now} · User: {user_dir.name}</p>
...
def main():
    parser = argparse.ArgumentParser(description="Generate static HTML career dashboard")
    parser.add_argument("--user", required=True, help="Username under users/")
    parser.add_argument("--output", default=None, help="Output path (default: users/<user>/dashboard.html)")
    parser.add_argument("--vscode", action="store_true", help="Include VS Code file URIs")
    ...
    output_path = Path(args.output) if args.output else user_dir / "dashboard.html"

    dashboard_html = generate_dashboard(user_dir, args.vscode)
